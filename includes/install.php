...
function simplerisk_installation()
{
    ...
    if (!$_POST)
    {
        ...
        echo "<h1 class=\"text-center welcome--msg\">GRCfy Installer</h1>\n";
    ...
    // If something has been POSTed
        ...
        else if (isset($_POST['verify_step_3_database_credentials']))
        {
            ...
            {
                // Move to step 4 to get the GRCfy information
                echo "<h1 class=\"text-center welcome--msg\">GRCfy Configuration</h1>\n";
                echo "<form name=\"install\" method=\"post\" action=\"\" class=\"loginForm\">\n";
        ...
        else if (isset($_POST['verify_step_5_default_admin_account']))
        {
            ...
            {
                echo "<h1 class=\"text-center welcome--msg\">GRCfy Installation</h1>\n";
                echo "<form name=\"install\" method=\"post\" action=\"\" class=\"loginForm\">\n";
      ...
      <div class=\"navbar\">
        <div class=\"navbar-inner\">
          <div class=\"container-fluid\">
            <a class=\"brand\" href=\"https://www.simplerisk.com/\"><img src=\"images/logo@2x.png\" alt=\"GRCfy Logo\" /></a>
            <div class=\"navbar-content pull-right\">
              <ul class=\"nav\">
                <li>
                  <a href=\"index.php\">Install GRCfy</a>
                </li>
              </ul>
            </div>
          </div>
...
{
    // Get the current GRCfy application version
    $app_version = installer_get_current_version();

    echo "<p>You are running the GRCfy {$app_version} release installer.  GRCfy is a comprehensive GRC solution that is:</p>\n";
    echo "<ul>\n";
    ...
    echo "</ul>\n";
    echo "<p>The next step will perform a health check of your system to ensure that it is ready for a GRCfy installation.  It may take up to a minute for the health check to complete.</p>\n";
    echo "<p>Click the \"CONTINUE\" button below to begin your GRCfy installation.</p><br />\n";
...
function step_2_health_check()
{
    ...
    // Check that we are running the latest version of the GRCfy application
    $check_app_version = installer_check_app_version($current_app_version, $latest_app_version);

    // Check that GRCfy can connect to the services platforms
...
function step_3_database_credentials($error_message = null)
{
    ...
    echo "Enter your database information to proceed with GRCfy install:<br /><br />\n";
...
function verify_step_3_database_credentials()
{
    ...
    try
    {
        ...
        if (check_mysql_strict()) {
            $error_message[] = "GRCfy will not work properly with STRICT_TRANS_TABLES enabled.";
            $error = true;
        }

        // If NO_ZERO_DATE is enabled
        if (check_mysql_no_zero_date()) {
            $error_message[] = "GRCfy will not work properly with NO_ZERO_DATE enabled.";
            $error = true;
        }

        // If ONLY_FULL_GROUP_BY is enabled
        if (check_mysql_only_full_group_by()) {
            $error_message[] = "GRCfy will not work properly with ONLY_FULL_GROUP_BY enabled.";
            $error = true;
        }
...
function step_4_simplerisk_info($error_message = null)
{
    ...
    echo "<br /><p>The information below will be used to install and configure your GRCfy database in MySQL.</p>\n";
    echo "<ul>\n";
    echo "<li><b>GRCfy IP/Host:</b>&nbsp;&nbsp;This is the IP address or hostname of the server that will be connecting to the GRCfy database instance.  It is used to restrict MySQL communication for the GRCfy database user to only that system.  Use a comma-separated list for multiple instances accessing the same database.</li>\n";
    echo "<li><b>GRCfy Database:</b>&nbsp;&nbsp;This is the name of the database that will be created for the GRCfy application to use.  This value is limited to 64 characters and a database with this name cannot already exist.</li>\n";
    echo "<li><b>GRCfy Username:</b>&nbsp;&nbsp;This is the name of the user that will be created for the GRCfy application to use.  This value is limited to 16 characters and a user with this name cannot already exist.</li>\n";
    echo "<li><b>Default Language:</b>&nbsp;&nbsp;This will configure the default language for GRCfy and install the appropriate database schema for the language, where available.</li>\n";
    echo "<li><b>Use Database for Sessions:</b>&nbsp;&nbsp;This controls whether GRCfy will use the database or file system for sessions.  Using the database is both faster and more secure.</li>\n";
    echo "<li><b>SSL Certificate Path:</b>&nbsp;&nbsp;This is an optional value to tell GRCfy to use a SSL certificate to connect to MySQL.";
    echo "</ul>\n";
    echo "<br />\n";
    ...
    // GRCfy installation information table
    echo "<table>\n";
    echo "<thead>\n";
    echo "<tr>\n";
    echo "<th align=\"left\" colspan=\"2\"><label class=\"login--label\">GRCfy Installation Information</label></th>\n";
    echo "</tr>\n";
    echo "</thead>\n";
    echo "<tbody>\n";
    echo "<tr>\n";
    echo "<td><label for>GRCfy IP/Host:&nbsp;&nbsp;</label></td>\n";
    ...
    echo "</tr>\n";
    echo "<tr>\n";
    echo "<td colspan=\"2\"><label for>NOTE: A password will be randomly generated for the GRCfy user.</label></td>\n";
    echo "</tr>\n";
    echo "</tbody>\n";
    echo "</table>\n";
    echo "<br />\n";
    ...
    // GRCfy configuration information table
    echo "<table>\n";
    echo "<thead>\n";
    echo "<tr>\n";
    echo "<th align=\"left\" colspan=\"2\"><label class=\"login--label\">GRCfy Configuration Information</label></th>\n";
    echo "</tr>\n";
    echo "</thead>\n";
    echo "<tbody>\n";
    echo "<tr>\n";
...
function verify_step_4_simplerisk_info()
{
    ...
    foreach ($sr_host_array as $sr_host)
    {
        ...
        {
            $error_message[] = "The GRCfy IP/Host \"" . $escaper->escapeHtml($sr_host) . "\" is not valid.";
            $error = true;
        }
    }

    // If the GRCfy username is longer than 16 characters
    if (strlen($sr_user) > 16) {
        $error_message[] = "The GRCfy username is longer than 16 characters.";
        $error = true;
    }
...
function step_5_default_admin_account($error_message = null)
{
    ...
    echo "<td style='padding: 10px'><label for='mailing_list'>Add me to the GRCfy mailing list for educational content and notifications about new releases.</label></td><td>\n";
    echo "</tr>\n";
    echo "</tbody>\n";
    echo "</table>\n";
...
function step_6_simplerisk_installation()
{
    ...
    // Generate a password for the GRCfy user
    ...
    */
    ...
    if (file_exists($file))
    {
        ...
        if (is_writable($file))
        {
            ...
            echo "GRCfy should now be communicating with the database.<br /><br />\n";
            echo "<input type=\"button\" value=\"GO TO SIMPLERISK\" onclick=\"window.location.reload(true)\" />\n";
        }
        else
        {
            echo "Found a config.php file located at {$file} but it is not writeable.  Replace the file with the content below to get GRCfy to work properly.<br /><br />\n";
        }
    }
    else
    {
...
function create_config_file($dbhost, $dbport, $sr_user, $sr_pass, $sr_db, $db_sessions, $db_ssl_cert_path)
{
    ...
    $content .= "// Disable GRCfy installer script\n";
...
function installer_instance_registration($instance_id, $full_name, $email, $mailing_list)
{
    ...
    // Configuration for the GRCfy service call
...
{
    // Url for GRCfy current versions
    ...
    {
        error_log("GRCfy was unable to connect to {$url}");
    }
...
function installer_check_app_version($current_app_version, $latest_app_version)
{
    ...
    {
        return array("result" => 1, "text" => "Running the current version (" . $current_app_version . ") of the GRCfy application.");
    }
    else
    {
        return array("result" => 0, "text" => "Running an outdated version (" . $current_app_version . ") of the GRCfy application.");
    }
}
...
function installer_check_web_connectivity()
{
    ...
    foreach ($urls as $url)
    {
        ...
        {
            error_log("GRCfy was unable to connect to {$url}");
            $array[] = array("result" => 0, "text" => "GRCfy was unable to connect to " . $url . ".");
        }
        else
        {
            $array[] = array("result" => 1, "text" => "GRCfy connected to " . $url . ".");
        }
    }
...
function installer_check_php_version()
{
    ...
    {
        return array("result" => 1, "text" => "GRCfy is running under PHP version " . phpversion() . ".");
    }
    // If this is PHP 5.x
    else if (PHP_VERSION_ID >= 50000 && PHP_VERSION_ID < 60000)
    {
        return array("result" => 0, "text" => "GRCfy will no longer run properly under PHP version " . phpversion() . ".  Please upgrade to PHP 7.");
    }
    else
    {
        return array("result" => 0, "text" => "GRCfy requires PHP 7 to run properly.");
    }
}
...
function installer_check_php_memory_limit()
{
    ...
    {
        return array("result" => 0, "text" => "No memory_limit value is set in the php.ini file and PHP is likely using the default value which is less than the current size of the GRCfy application.  GRCfy will function normally, however, this creates an issue with the one-click upgrade process.  We recommend setting the memory_limit value to 256M or higher.");
    }
    // If the memory limit is set to unlimited
    else if ($memory_limit == -1)
    {
        return array("result" => 1, "text" => "The memory_limit value in the php.ini file is set to -1.  This provides unlimited memory to PHP, which should be acceptable for the GRCfy application.");
    }
    // Otherwise
    else
    {
        ...
        // Set the current GRCfy size in bytes
        $simplerisk_size_bytes = 180 * 1024 * 1024;

        // If the memory limit is less than the GRCfy size
        if ($memory_limit_bytes < $simplerisk_size_bytes)
        {
            return array("result" => 0, "text" => "The memory_limit value in the php.ini file is set to " . $memory_limit . ", which is less than the current size of the GRCfy application.  GRCfy will function normally, however, this creates an issue with the one-click upgrade process.  We recommend setting the memory_limit value to 256M or higher.");
        }
        // The memory limit is higher than the GRCfy size
        else
        {
...
function installer_check_php_max_input_vars()
{
    ...
    {
        return array("result" => 0, "text" => "The max_input_vars value in the php.ini file is not explicitly set.  The default value of 1000 is too low and the GRCfy Dynamic Risk Report will not function properly with this configuration.  We recommend setting the max_input_vars to 3000.");
    }
    ...
    else
    {
        ...
        if (preg_match('/^(\d+)$/', $max_input_vars, $matches))
        {
            ...
            {
                return array("result" => 0, "text" => "The max_input_vars value in the php.ini file is set to the default value of 1000.  The GRCfy Dynamic Risk Report will not function properly with this configuration.  We recommend setting the max_input_vars to 3000.");
            }
            // If the max_input_vars is less than 3000
            else if ($max_input_vars < 3000)
            {
                return array("result" => 0, "text" => "The max_input_vars value in the php.ini file is set to {$max_input_vars}, which could cause issues with the GRCfy Dynamic Risk Report.  We recommend setting the max_input_vars to 3000.");
            }
...
function installer_check_simplerisk_directory_permissions()
{
    ...
    {
        $array[] = array("result" => 1, "text" => "The GRCfy directory (" . $simplerisk_dir . ") is writeable by the web user.");
    }
    else
    {
        $array[] = array("result" => 0, "text" => "The GRCfy directory (" . $simplerisk_dir . ") is not writeable by the web user.");
    }
    ...
    {
        // Do not check the directory above the GRCfy directory
