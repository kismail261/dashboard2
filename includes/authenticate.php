...
function is_valid_user($user, $pass, $upgrade = false)
{
    ...
    {
        // Write the debug log
        write_debug_log("Not a valid user in GRCfy.");
    ...
    // If either the SAML, AD, or GRCfy user are valid
...
//        }
    ...
    // Get the latest GRCfy version and add it to the session
...
function send_reset_email($username, $name, $email, $token)
{
    ...
    // Additional headers
    $headers .= "From: GRCfy <noreply@simplerisk.com>\r\n";
    $headers .= "Reply-To: GRCfy <noreply@simplerisk.com>\r\n";
    $headers .= "X-Mailer: PHP/" . phpversion();

    // Create the full HTML message
    $body = "<html><body>\n";
    $body .= "<p>Hello " . $escaper->escapeHtml($name).",</p>\n";
    $body .= "<p>A request was submitted to reset your GRCfy password.</p>\n";
    ...
    if(get_setting('simplerisk_base_url') != false) {
        ...
        $body .= "<p>You may now use the \"<a href='{$base_url}'>Forgot your password</a>\" link on the GRCfy log in page to reset your password.</p>";
    } else {
        $body .= "<p>Please visit the GRCfy log in page to reset your password.</p>";
    }
