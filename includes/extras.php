 ...
 * Returns an array of available GRCfy Extras *
 ***************************************************/
function available_extras()
{
    // The available GRCfy Extras
    $extras = array(
 ...
 * Returns the short names of available GRCfy Extras *
 **********************************************************/
...
function core_display_upgrade_extras()
{
	...
	echo "<p><h4>" . $escaper->escapeHtml($lang['GRCfyExtras']) . "</h4></p>\n";
	...
	else
	{
		// Display the table row
		echo "<tr>\n";
		echo "  <td colspan=\"8\"><b>" . $escaper->escapeHtml($lang['UnableToCommunicateWithTheGRCfyServer']) . "</b></td>\n";
		echo "</tr>\n";
	}
...
function core_is_purchased($extra)
{
    ...
    // If the GRCfy service call returned false
    if ($return_code !== 200)
    {
...
function core_check_all_purchases()
{
    ...
    {
        write_debug_log("GRCfy successfully connected to " . $url);
...
function extra_simplerisk_version_compatible($extra)
{
	...
	else
	{
		...
		// Get the current version of GRCfy
		$simplerisk_version = current_version("app");
		write_debug_log("The current version of GRCfy is " . $simplerisk_version);
		...
		else
		{
			...
			foreach($array[$extra]['extra'] as $key => $value)
			{
				...
				{
					write_debug_log("The current version of GRCfy is compatible with this Extra.");
					return true;
				}
			}

			// If we never found our match
			write_debug_log("The current version of GRCfy is not compatible with this Extra.");
			return false;
		}
	}
}
	...
	if ($purchases != false)
	{
		...
		foreach ($extras as $extra)
		{
			...
			if ($extra['short_name'] != "upgrade" && $extra['short_name'] != "complianceforgescf")
			{
				...
				{
					write_debug_log("GRCfy says this Extra should be disabled: " . $extra['short_name']);
				...
				{
					write_debug_log("GRCfy says this Extra should be deleted: " . $extra['short_name']);
