...
function db_open()
{
    ...
    catch (PDOException $e)
    {
        ...
        // We were unable to connect to the GRCfy database
        require_once(realpath(__DIR__ . '/healthcheck.php'));
...
function upload_file($risk_id, $file, $view_type = 1)
{
    ...
    if (isset($file) && $file['name'] != "")
    {
        ...
        if (in_array($file['type'], $allowed_types))
        {
            ...
            else return "The uploaded file was too big to store in the database.  A GRCfy administrator can modify the maximum file upload size under \"File Upload Settings\" under the \"Configure\" menu.  You may also need to modify the 'upload_max_filesize' and 'post_max_size' values in your php.ini file.";
            }
            else return "The file extension of the uploaded file (" . pathinfo($file['name'], PATHINFO_EXTENSION) . ") is not supported.  A GRCfy administrator can add it under \"File Upload Settings\" under the \"Configure\" menu.";
        }
        else return "The file type of the uploaded file (" . $file['type'] . ") is not supported.  A GRCfy administrator can add it under \"File Upload Settings\" under the \"Configure\" menu.";
    }
    else return 1;
}
...
function add_registration($name="", $company="", $title="", $phone="", $email="", $fname="", $lname="")
{
    ...
    // If there was an error communicating with the GRCfy services API
    if ($return_code !== 200)
    {
        write_debug_log("Unable to communicate with the GRCfy services API");
    ...
    {
        write_debug_log("Successfully made the GRCfy service call");
        set_alert(true, "good", "Successfully made the GRCfy service call");

        $results = $response['response'];
        $results = array($results);

        // For each line in the results returned from the GRCfy service call
        foreach ($results as $line) {
            if (preg_match("/<api_key>(.*)<\/api_key>/", $line, $matches)) {
                write_debug_log("An API key was returned from the GRCfy services tier");
                set_alert(true, "good", "An API key was returned from the GRCfy services tier");
...
function update_registration($name="", $company="", $title="", $phone="", $email="", $fname="", $lname="")
{
    ...
    // If the GRCfy service call failed
    if ($return_code !== 200) {
...
{
    // If the GRCfy instance is not registered
    if (get_setting('registration_registered') == 0)
    {
        ...
        if (isset($_SESSION["admin"]) && $_SESSION["admin"] == "1")
        {
            ...
            {
                // Set the alert
                set_alert(true, "good", "You haven't registered GRCfy yet.  Register now to be able to back up and upgrade with the click of a button.");
...
//                                $errors[] = "No file was uploaded.";
                ...
                else $errors[] = "The uploaded file was too big to store in the database.  A GRCfy administrator can modify the maximum file upload size under \"File Upload Settings\" under the \"Configure\" menu.  You may also need to modify the 'upload_max_filesize' and 'post_max_size' values in your php.ini file.";
                }
                else $errors[] = "The file extension of the uploaded file (" . pathinfo($file['name'], PATHINFO_EXTENSION) . ") is not supported.  A GRCfy administrator can add it under \"File Upload Settings\" under the \"Configure\" menu.";
            }
            else $errors[] = "The file type of the uploaded file (" . $file['type'] . ") is not supported.  A GRCfy administrator can add it under \"File Upload Settings\" under the \"Configure\" menu.";
...
function get_mysqldump_command()
{
    ...
    {
        // Get the path from the GRCfy configuration
