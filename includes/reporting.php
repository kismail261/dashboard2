...
function open_risk_level_pie_new($title = null, $teams = false, $score_used='inherent') {
    ...
    if (!empty($array)) {
        ...
        $labelsArray = array();
        foreach ($array as $row) {
            if ($array[0]['level'] == 'High') {
                $close = $row['num'];
            } else if ($array[0]['level'] == 'Medium') {
                $open = $row['num'];
            }
            array_push($seriesArray, $row['num']);
            array_push($labelsArray, $array[0]['level']);
        }
    }
    ...
    $high_percentage= 0;
    $medium_percentage = 0;
    if(count($chart_series) > 0){
        if($chart_series[0]['data'][0][0] == 'Medium'){
            $medium =  $chart_series[0]['data'][0][1];
            $medium_percentage = 100;
        } else {
            $high =  $chart_series[0]['data'][0][1];
            $mediums =  $chart_series[0]['data'][1][1];
            $total = $high + $mediums;
    ...
    if(count($chart_series) > 0){
        if(count($chart_series) > 0){
            if($chart_series[0]['data'][0][0] == 'Medium'){
                ...
                $high =  $chart_series[0]['data'][0][1];
                $medium =  $chart_series[0]['data'][1][1];
                $total = $high + $medium;
    ...
    echo "<div id=\"open_closed_donut\"></div>\n";
    echo '<script src="../zem_glass_assets/assets/js/apexcharts.js"></script>';
    echo "<script>";
    echo "var options = " . $json . ";";
    echo "var chart = new ApexCharts(document.querySelector('#open_closed_donut'), options);";
    echo "chart.render();";
    echo "</script>";
    echo'<div class="d-flex mg-b-15">';
    echo'<div class="me-2">';
    echo'<span class="avatar avatar-sm radius-4 bg-primary-transparent tx-primary"><i class="fe fe-alert-circle"></i></span>';
    echo'</div>';
    echo'<div class="flex-1">';
    echo'<div class="flex-between mb-1">';
    echo'<p class="mb-0"><span class="pe-2 border-end">High</span><span class="ps-2 tx-muted">'. (int)$high_percentage .'%</span></p>';
    echo'</div>';
    echo'<div class="progress">';
    echo'<div class="progress-bar progress-bar-striped progress-bar-animated ht-5 bg-primary" style="width: '. (int)$high_percentage .'%"></div>';
    echo'</div>';
    echo'</div>';
    echo'</div>';
    echo'<div class="d-flex mg-b-15">';
    echo'<div class="me-2">';
    echo'<span class="avatar avatar-sm radius-4 bg-secondary-transparent tx-secondary"><i class="fe fe-check"></i></span>';
    echo'</div>';
    echo'<div class="flex-1">';
    echo'<div class="flex-between mb-1">';
    echo'<p class="mb-0"><span class="pe-2 border-end">Medium</span><span class="ps-2 tx-muted">'. (int)$medium_percentage .'%</span></p>';
    echo'</div>';
    echo'<div class="progress">';
    echo'<div class="progress-bar progress-bar-striped progress-bar-animated ht-5 bg-secondary" style="width: '. (int)$medium_percentage .'%"></div>';
    echo'</div>';
    echo'</div>';
    echo'</div>';
}
...
function risks_by_month_table()
{
    ...
    echo "<thead>\n";
    echo "<tr bgcolor=\"white\" style='background: #182E37;'>\n";
    echo "<th>&nbsp;</th>\n";
...
/*****************************
 * FUNCTION: OPEN CLOSED DONUT *
 *****************************/
function open_close_donut($title = null){
    
    // Create the chart options array
    $options = array(
        'plotOptions' => array(
            'type' => 'donut',
            'renderTo' => 'open_closed_donut',
            'height' => 300
        ),
        'title' => array(
            'text' => $title
        ...
        'plotOptions' => array(
            'pie' => array(
                ...
                'colors' => array('green', 'red') // Define colors for the slices (Closed = green, Open = red)
            )
        ),
        'series' => array(
            array(
                'name' => 'Status',
                'data' => array(
                    array('Closed', 0), // Set initial values for Closed and Open slices
                    array('Open', 0)
                )
            )
        )
    ...
    if (!team_separation_extra()) {
        ...
        // Query the database
        $stmt = $db->prepare("SELECT COUNT(*) AS num, CASE WHEN status = 'Closed' THEN 'Closed' WHEN status != 'Closed' THEN 'Open' END AS name FROM `risks` GROUP BY name ORDER BY name");
        $stmt->execute();
    ...
    if (!empty($array)) {
        ...
        foreach ($array as $row) {
            ...
            array_push($seriesArray, $row['num']);
            array_push($labelsArray, $row['name']);
        }
    }
    $data = array(
        'plotOptions' => array(
    ...
    echo'<div class="flex-1">';
    echo'<div class="flex-between mb-1">';
    echo'<p class="mb-0"><span class="pe-2 border-end">Opened</span><span class="ps-2 tx-muted">'. $openPer .'</span></p>';
    echo'</div>';
    echo'<div class="progress">';
    echo'<div class="progress-bar progress-bar-striped progress-bar-animated ht-5 bg-primary" style="width: '. $openPer .'"></div>';
    echo'</div>';
    echo'</div>';
    echo'</div>';
    echo'<div class="d-flex mg-b-15">';
    ...
    echo'<div class="flex-1">';
    echo'<div class="flex-between mb-1">';
    echo'<p class="mb-0"><span class="pe-2 border-end">Closed</span><span class="ps-2 tx-muted">'. $closePer .'</span></p>';
    echo'</div>';
    echo'<div class="progress">';
    echo'<div class="progress-bar progress-bar-striped progress-bar-animated ht-5 bg-secondary" style="width: '. $closePer .'"></div>';
    echo'</div>';
    echo'</div>';
    echo'</div>';
}

function new_mitigation_donut($title = null){
    $chartOptions = [
        "chart" => [
            "renderTo" => "open_mitigation_pie",
        ...
        "plotOptions" => [
            "pie" => [
                "point" => [
                    "events" => [
                        "click" => new HighchartJsExpr("function() {
                            location.href = 'dynamic_risk_report.php?status=2&group=2&sort=0';
                        }"),
                    ],
                ],
    ...
    $labelsArray = array();
    foreach ($array as $row) {
        $data[] = [(string)$row['name'], (int)$row['num']];
        
        if ($row['name'] == 'Unplanned') {
            $close = $row['num'];
        } else if ($row['name'] == 'Planned') {
            $open = $row['num'];
        }
        array_push($seriesArray, $row['num']);
        array_push($labelsArray, $row['name']);
    ...
    $chartOptions["plotOptions"]["pie"]["colors"] = $color_array;
    $chartOptions["series"] = [
        [
            "type" => "pie",
            "name" => "Status",
    ...
    echo'<script>';
    echo'var options ='. $json . ';';
    echo'var chart = new ApexCharts(document.querySelector("#open_mitigation_pie"), options);';
    echo'chart.render();';
    echo'</script>';
    echo'<div class="d-flex mg-b-15">';
    echo'<div class="me-2">';
    echo'<span class="avatar avatar-sm radius-4 bg-primary-transparent tx-primary"><i class="fe fe-eye"></i></span>';
    echo'</div>';
    echo'<div class="flex-1">';
    echo'<div class="flex-between mb-1">';
    echo'<p class="mb-0"><span class="pe-2 border-end">Planned</span><span class="ps-2 tx-muted">'. round($openPer) .'%</span></p>';
    echo'</div>';
    echo'<div class="progress">';
    echo'<div class="progress-bar progress-bar-striped progress-bar-animated ht-5 bg-primary" style="width: '. round($openPer) .'%"></div>';
    echo'</div>';
    echo'</div>';
    echo'</div>';
    echo'<div class="d-flex mg-b-15">';
    echo'<div class="me-2">';
    echo'<span class="avatar avatar-sm radius-4 bg-secondary-transparent tx-secondary"><i class="fe fe-eye-off"></i></span>';
    echo'</div>';
    echo'<div class="flex-1">';
    echo'<div class="flex-between mb-1">';
    echo'<p class="mb-0"><span class="pe-2 border-end">Unplanned</span><span class="ps-2 tx-muted">'. round($closePer) .'%</span></p>';
    echo'</div>';
    echo'<div class="progress">';
    echo'<div class="progress-bar progress-bar-striped progress-bar-animated ht-5 bg-secondary" style="width: '. round($closePer) .'%"></div>';
    echo'</div>';
    echo'</div>';
    echo'</div>';
}

function new_review_donut($title = null)
{
    $chartOptions = [
        "chart" => [
            "renderTo" => "open_review_pie",
            "plotBackgroundColor" => null,
            "plotBorderWidth" => null,
            "plotShadow" => false,
        ],
    ...
    echo'<script>';
    echo'var options = '. $json .';';
    echo'var chart = new ApexCharts(document.querySelector("#open_review_pie"), options);';
    echo'chart.render();';
    echo'</script>';
    echo'<div class="d-flex mg-b-15">';
    echo'<div class="me-2">';
    ...
    echo'<div class="flex-1">';
    echo'<div class="flex-between mb-1">';
    echo'<p class="mb-0"><span class="pe-2 border-end">Reviewed</span><span class="ps-2 tx-muted">'. round($openPer) .'%</span></p>';
    echo'</div>';
    echo'<div class="progress">';
    echo'<div class="progress-bar progress-bar-striped progress-bar-animated ht-5 bg-primary" style="width: '. round($openPer) .'%"></div>';
    echo'</div>';
    ...
    echo'<div class="flex-1">';
    echo'<div class="flex-between mb-1">';
    echo'<p class="mb-0"><span class="pe-2 border-end">Unreviewed</span><span class="ps-2 tx-muted">'. round($closePer) .'%</span></p>';
    echo'</div>';
    echo'<div class="progress">';
    echo'<div class="progress-bar progress-bar-striped progress-bar-animated ht-5 bg-secondary" style="width: '. round($closePer) .'%"></div>';
    echo'</div>';
