...
function simplerisk_health_check()
{
	...
	// Check that we are running the latest version of the GRCfy database
	...
	// Check the Extra versions match the GRCfy version
	...
	// Check the GRCfy Base URL
	$check_simplerisk_base_url = check_simplerisk_base_url();

	// Check the GRCfy database connectivity
	$check_database_connectivity = check_database_connectivity();

	// Check that GRCfy can communicate with the API
	...
	echo "    <li><a href=\"#/simplerisk\">GRCfy Core</a></li>\n";
	...
	{
		health_check_good("GRCfy Core");
	}
	else health_check_bad("GRCfy Core");
	...
	echo "        </tr>\n";
	echo "        <tr>\n";
	echo "          <td style='padding-right:25px;'><b>GRCfy Version:</b></td>\n";
	echo "          <td>" . $escaper->escapeHtml($current_app_version) . "</td>\n";
	echo "        </tr>\n";
	echo "        <tr>\n";
	echo "          <td style='padding-right:25px;'><b>GRCfy Instance ID:</b></td>\n";
...
function check_db_version($current_db_version, $latest_db_version)
{
	// If the current and latest versions are the same
        ...
        {
		return array("result" => 1, "text" => "Running the current version (" . $current_db_version . ") of the GRCfy database schema.");
        }
        else
        {
		return array("result" => 0, "text" => "Running an outdated version (" . $current_db_version . ") of the GRCfy database schema.");
        }
}
...
function check_same_app_and_db($current_app_version, $current_db_version)
{
	...
	{
		return array("result" => 1, "text" => "The GRCfy application and database are at the same versions (" . $current_db_version . ").");
	}
	else
	{
		return array("result" => 0, "text" => "The GRCfy application (" . $current_app_version . ") and database (" . $current_db_version . ") versions are not the same.");
	}
}
...
function check_web_connectivity()
{
	...
	foreach ($urls as $url)
	{
		...
		{
			write_debug_log("GRCfy was unable to connect to " . $url);
			$array[] = array("result" => 0, "text" => "GRCfy was unable to connect to " . $url . ".");
		}
		else
		{
			write_debug_log("GRCfy connected to " . $url);
...
{
	// Get the GRCfy Base URL value from the database
	...
	{
		return array("result" => 1, "text" => "Your GRCfy Base URL matches the URL you are using to connect to GRCfy.");
	}
	else
	{
		return array("result" => 0, "text" => "Your GRCfy Base URL does not match the URL you are using to connect to GRCfy.");
	}
}
...
function check_database_connectivity()
{
	...
	if ($db !== null)
	{
		...
		return array("result" => 1, "text" => "Communicated with the GRCfy database successfully.");
	}
	else
	{
		return array("result" => 0, "text" => "Unable to communicate with the GRCfy database.");
	}
}
...
function check_api_connectivity()
{
	...
	else
	{
		...
		// If SSL certificate checks are enabled for the GRCfy API
		...
		if ($return_code === 200)
		{
			...
			if (!$validate_ssl)
			{
				return array("result" => 1, "text" => "Communicated with the GRCfy API successfully, but SSL certificate checks are disabled.");
			}
			else
			{
				return array("result" => 1, "text" => "Communicated with the GRCfy API successfully.");
			}
		}
		else
		{
			...
			if (!$validate_ssl)
			{
				return array("result" => 0, "text" => "Unable to communicate with the GRCfy API even with SSL certificate checks disabled.");
			}
			else
			{
				return array("result" => 0, "text" => "Unable to communicate with the GRCfy API.  To debug, try disabling SSL certificate checks for GRCfy API requests.");
			}
		}
	}
}
...
function check_strict_sql_mode()
{
	...
	{
		return array("result" => 0, "text" => "GRCfy will not work properly with STRICT_TRANS_TABLES enabled.");
	}
	else
	{
...
function check_no_zero_date()
{
        ...
        {       
		return array("result" => 0, "text" => "GRCfy will not work properly with NO_ZERO_DATE enabled.");
        }
        else    
        {       
...
function check_mysql_size()
{
	...
	return array("result" => 1, "text" => "GRCfy is using " . round($size, 2) . " MB of disk space.");
}
...
function check_only_full_group_by()
{
        ...
        {       
		return array("result" => 0, "text" => "GRCfy will not work properly with ONLY_FULL_GROUP_BY enabled.");
        }
        else    
        {       
...
function check_extra_versions($current_app_version)
{
	global $escaper;
		...
		if (file_exists(realpath(__DIR__ . '/../extras/upgrade/index.php')))
		{
			...
			// Get the list of available GRCfy Extras
			...
			foreach ($extras as $extra)
			{
				...
				if ($purchased)
				{
					...
					if (core_is_installed($extra['short_name']))
					{
						...
						if (!$expired)
						{
							$array[] = array("result" => 1, "text" => "The GRCfy " . $escaper->escapeHtml($extra['long_name']) . " has been purchased and installed.");
						}
						// The license has expired
						else
						{
							$array[] = array("result" => 0, "text" => "Your license for the GRCfy " . $escaper->escapeHtml($extra['long_name']) . " has expired.");
						}

						// If this extra is compatible with this version of GRCfy
						if (extra_simplerisk_version_compatible($extra['short_name']))
						{
							$array[] = array("result" => 1, "text" => "The currently installed " . $escaper->escapeHtml($extra['long_name']) . " is compatible with this version of GRCfy.");
						}
						// This extra is not compatible
						else
						{
							$array[] = array("result" => 0, "text" => "The currently installed " . $escaper->escapeHtml($extra['long_name']) . " is not compatible with this version of GRCfy.");
						}
					...
					{
						$array[] = array("result" => 0, "text" => "The GRCfy " . $escaper->escapeHtml($extra['long_name']) . " has been purchased but is not installed.");
					}
				}
			}
		}
...
function check_use_database_for_session()
{
	...
	if (defined('USE_DATABASE_FOR_SESSIONS'))
	{
		...
		{
			return array("result" => 0, "text" => "The USE_DATABASE_FOR_SESSIONS value is set to false in the config.php file.  GRCfy will function normally, however, this creates an issue with the one-click upgrade process.  We recommend setting the USE_DATABASE_FOR_SESSIONS to true.");
		}
...
<html ng-app=\"GRCfy\">
  <head>
    <title>GRCfysss: Enterprise Risk Management Simplified</title>
