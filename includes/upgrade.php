...
function display_login_form()
{
    ...
    echo "<h1 class=\"text-center welcome--msg\">Upgrade the GRCfy Database </h1>";
...
function display_upgrade_info()
{
    ...
    echo "This script will ugprade your database to the next version of GRCfy.  Please make sure you have backed up your database before proceeding.  Click &quot;CONTINUE&quot; to begin.<br />\n";
        echo "<br />\n";
...
function upgrade_from_20140728001($db)
{
    ...
    echo "Beginning GRCfy database upgrade from version " . $version_to_upgrade . " to version " . $version_upgrading_to . "<br />\n";
    ...
    echo "Finished GRCfy database upgrade from version " . $version_to_upgrade . " to version " . $version_upgrading_to . "<br />\n";
}
...
function upgrade_from_20150321001($db)
{
    ...
    // Create a random id for this GRCfy instance
...
function upgrade_from_20150531001($db)
{
    ...
    echo "Adding a setting to show GRCfy is not registered.<br />\n";
...
function upgrade_from_20160124001($db)
{
    ...
    $stmt->execute();
    $stmt = $db->prepare("INSERT INTO `settings` VALUES ('phpmailer_from_name', 'GRCfy');");
    $stmt->execute();
    $stmt = $db->prepare("INSERT INTO `settings` VALUES ('phpmailer_replyto_email', 'noreply@simplerisk.com');");
    $stmt->execute();
    $stmt = $db->prepare("INSERT INTO `settings` VALUES ('phpmailer_replyto_name', 'GRCfy');");
    $stmt->execute();
...
function upgrade_from_20190105001($db){
    ...
    if (!get_setting('simplerisk_base_url') && isset($_SERVER) && array_key_exists('SERVER_NAME', $_SERVER)) {
        echo "Setting the default value for the GRCfy Base URL.<br />\n";
...
function upgrade_from_20190331001($db){
    ...
    // If the GRCfy instance is registered
        ...
        'governance' => [
            ...
            'permissions' => [
                'governance' => [
                    ...
                    'description' => 'This permission grants a user access to the "Governance" menu in GRCfy.',
                    'order' => 1,
                ],
                'add_new_frameworks' => [
        ...
        'assessments' => [
            ...
            'permissions' => [
                'assessments' => [
                    ...
                    'description' => 'This permission will allow the user access to the "Assessments" menu. If GRCfy has the "Risk Assessment Extra" enabled the user will gain access to this as well with all the permssions to make, change, and send assessments.',
                    'order' => 1
                ]
            ]
        ]
...
function upgrade_from_20221013001($db)
{
    ...
    {
        // Remove the table that is not being used by the GRCfy Core
...
function upgrade_database()
{
    ...
    if (check_grants($db))
    {
        ...
        if ($app_version != $db_version)
        {
            ...
            else echo "Unable to find an upgrade function for the current GRCfy database version.<br />\n";
        }
        // The applicaton and database are updated to the same version
        else
        {
            echo "You are currently running the version of the GRCfy database that goes along with your application version.<br />\n";
        }
    }
