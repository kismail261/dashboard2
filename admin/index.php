...
if (isset($_POST['update_general_settings']))
{
    ...
    if (isset($_POST['delete_file_type']))
    {
        ...
        if (isset($_POST['update_max_upload_size']))
        {
                   ...
                   if (is_numeric($_POST['size']))
                   {
                           ...
                           // Get the currently set max upload size for GRCfy
                           $simplerisk_max_upload_size = get_setting('max_upload_size');

                           // If the max upload size for GRCfy is bigger than the PHP max upload size
                           if ($simplerisk_max_upload_size > php_max_allowed_values())
                           {
                                   // Display an alert
                                   set_alert(true, "bad", $escaper->escapeHtml($lang['WarnPHPUploadSize']));
                           }
                           // If the max upload size for GRCfy is bigger than the MySQL max_allowed_packet
        ...
        if (isset($_POST['test_mail_configuration']))
        {
                ...
                $name = "GRCfy Test";
                $email = $_POST['email'];
                $subject = "GRCfy Test Email";
                $full_message = "This is a test email from GRCfy.";
    ...
    if (isset($_POST['submit_backup']) || isset($_POST['submit_and_backup_now'])) {
        ...
        // Update the SSL certificate check for the GRCfy API
            ...
            <div id="content">
              ...
              <div id="files" style="display: none;" class="settings_tab">
                ...
                <?php
                        ...
                        // If the max upload size for GRCfy is bigger than the MySQL max upload size
        ...
        ?>
                  ...
                  <tr>
                    <td>
                      ...
                      <table name="backup" id="backup" border="0">
                        <tr>
                          <td colspan="2"><input type="checkbox" name="backup_auto" id="backup_auto" <?php echo ($backup_auto == "true") ? "checked=\"yes\" " : ""?>/>&nbsp;&nbsp;<?php echo $escaper->escapeHTML($lang['AutomaticallyBackupThisGRCfyInstance']); ?></td>
                        </tr>
                        <tr>
                      ...
                      <td>
                        <table border="0" width="100%">
                          <tbody>
                            ...
                            <tr>
                                <td colspan="2"><input <?php if($escaper->escapeHtml(get_setting('ssl_certificate_check_simplerisk')) == 1){ echo "checked"; } ?> name="ssl_certificate_check_simplerisk" class="hidden-checkbox" size="2" value="90" id="ssl_certificate_check_simplerisk" type="checkbox">  <label for="ssl_certificate_check_simplerisk"  >&nbsp;&nbsp; <?php echo $escaper->escapeHtml($lang['EnableSSLCertificateCheckGRCfy']); ?></label></td>
                            </tr>
                            <tr>
